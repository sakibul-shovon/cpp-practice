...
ll minDiffPiles(vll &weights,ll pos,ll sum1,ll totalSum){
    if(pos == weights.size()){
        return abs(sum1 - (totalSum - sum1));
    }

    ll include = minDiffPiles(weights,pos+1,sum1+weights[pos],totalSum);
    ll exclude = minDiffPiles(weights,pos+1,sum1,totalSum);
...
int main()
{
    ...
    ll ans = minDiffPiles(weights, 0, 0, totalSum);
    cout<<ans<<endl;
