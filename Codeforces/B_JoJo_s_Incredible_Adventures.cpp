// File Name: B_JoJo_s_Incredible_Adventures.cpp
// Date: 2025-11-18
// Time: 23:30:51
...
int main()
{
    ...
    While(T)
    {
        string s;
        cin >> s;
        //ai mere code korsi . kisu korar nai. partesina
        ll n = (ll)s.size();   // force long long

        if (count(all(s), '1') == n) {
            cout << n * n << '\n';   // now n*n is 64-bit, no overflow
            continue;
        }
        ll count = 0;
        ll maxx = 0;
