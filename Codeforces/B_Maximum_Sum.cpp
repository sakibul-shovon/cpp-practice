...
int main()
{
    ...
    While(T)
    {
        ...
        for (ll i = 0; i <= k; i++)
        {
            ...
            ll firstChoice = i;
            ll secondChoice = k - i;
            //cout<<firstChoice<<' '<<secondChoice<<endl;

            ll a =0,b = 0;
            if(firstChoice!=0){
                a = pref[2*firstChoice -1];
                
            }
            if(secondChoice !=0){
                b= pref[n-1] - pref[n-1-secondChoice];
            }
            //debug(a);debug(b);line;
            ll calc = pref[n-1] - (a+b);
            ans = max(ans,calc);
        }
        cout<<ans<<endl;
    }
