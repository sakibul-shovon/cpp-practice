// File Name: B_Make_it_Divisible_by_25.cpp
// Date: 2024-11-29
// Time: 03:58:33
...
int main() {
    ...
    while (t--) {
        ...
        ll minSteps = LLONG_MAX;

        
        for (auto target : {"00", "25", "50", "75"}) {
            ll pos1 = -1, pos2 = -1;

            
            for (ll i = 0; i < s.size(); i++) {
                if (pos1 == -1 && s[i] == target[1]) {
                    pos1 = i; 
                } else if (pos1 != -1 && s[i] == target[0]) {
                    pos2 = i; 
                    break;
                }
            }
