// File Name: B_A_and_B_and_Compilation_Errors.cpp
// Date: 2025-12-27
// Time: 16:39:32
...
int main()
{
    fastio;
    ll n;cin>>n;
    vll v1(n,inf),v2(n,inf),v3(n,inf);
    
    for(ll i=0;i<n;i++) cin>>v1[i];
    for(ll i=0;i<n-1;i++) cin>>v2[i];
    for(ll i=0;i<n-2;i++) cin>>v3[i];
    sort_all(v1);sort_all(v2);sort_all(v3);
    //autoLoop(v1);line;autoLoop(v2);line;autoLoop(v3);
