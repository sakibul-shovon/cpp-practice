// File Name: C_Number_of_Pairs.cpp
// Date: 2024-11-15
// Time: 00:15:53
...
int main()
{
    fastio;
    While(t){
        ll n,first,second;cin>>n>>first>>second;
        vll v(n);
        for(ll i=0;i<n;i++){
            cin>>v[i];
        }
        ...
        for(ll i=0;i<n;i++){
            auto lb = lower_bound(v.begin()+i+1,v.end(),first-v[i]);
            auto hb = upper_bound(v.begin()+i+1,v.end(), second - v[i]);
