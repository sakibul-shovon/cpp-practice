// File Name: B_Little_Elephant_and_Cards.cpp
// Date: 2024-05-19
// Time: 18:13:03
...
bool canDo(const vector<pair<ll, ll>> &cards, ll color, ll maxFlip)
{
    ll front = 0, back = 0;
    ...
    for (const auto &card : cards)
    {
        if (card.first == color)
            front++;
        if (card.second == color)
            back++;
    }

    ll total = front + min(back, maxFlip);
    return total >= (cards.size() + 1) / 2;
}
