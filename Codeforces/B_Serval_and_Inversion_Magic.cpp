// File Name: B_Serval_and_Inversion_Magic.cpp
// Date: 2025-12-03
// Time: 00:09:16
...
int main()
{
    ...
    While(T)
    {
        ...
        bool tick = false;
        bool second_cross = false;

        for (ll i = 0; i < x; i++)
        {
            if (!first_cross and s[i] != s[n - i - 1])
            {
                first_cross = true;
            }

            else if (first_cross and s[i] == s[n - i - 1])
            {
                tick = true;
            }
            else if (tick and s[i] != s[n - i - 1])
            {
                second_cross = true;
            }
        }

        if (first_cross and second_cross and tick)
        {
            cout << "No" << endl;
        }
        else
